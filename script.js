...
// DEMARRAGE DU JEU (Cache l'accueil déjà présent)
function startGame() {
...
function updateGold(amount) {
    ...
    // Si on ajoute de l'or, on débloque l'interface (Manœuvre)
...
// VERROUILLAGE / DEVERROUILLAGE DE L'INTERFACE
function applyInterfaceLock() {
    ...
    document.querySelectorAll('#prestigeSection button, #turnSection button, #iaButtonGroup button').forEach(b => {
        if (b.id === 'actionBtn' && iaHasPassedDefinitively) {
            b.disabled = true;
        } else {
            b.disabled = lock;
        }
...
// GESTION DE L'AIDE (FETCH RESTE OK POUR L'AIDE)
